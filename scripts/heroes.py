...
class StimManager:
	def __init__(self, level, tower_position, gf):
		...
		self.rect.x = gf.castle.castle_rect.x + 5*OBJECT_MULTIPLYER_WIDTH + ((self.size[0]+5*OBJECT_MULTIPLYER_WIDTH)*(tower_position-1))
		self.rect.y = gf.castle.castle_rect.y - 5*OBJECT_MULTIPLYER_HEIGHT - ((self.size[1]+5*OBJECT_MULTIPLYER_HEIGHT)*(tower_position)) + CASTLE_SIZE[1]
...
class Maradauer:
	def __init__(self, level, tower_position, gf):
		...
		self.rect = self.image.get_rect()
		self.rect.x = gf.castle.castle_rect.x + 5*OBJECT_MULTIPLYER_WIDTH + ((self.size[0]+5*OBJECT_MULTIPLYER_WIDTH)*(tower_position-1))
		self.rect.y = gf.castle.castle_rect.y - 5*OBJECT_MULTIPLYER_HEIGHT - ((self.size[1]+5*OBJECT_MULTIPLYER_HEIGHT)*(tower_position-1)) + CASTLE_SIZE[1]
	...
	def action(self, gf):
		if gf.in_battle and self.cooldown_iteration >= self.cooldown:
			...
			gf.change_unit(self.tower_position)
